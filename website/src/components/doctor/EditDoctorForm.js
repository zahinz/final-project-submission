...
export default function EditDoctorForm(props) {
  // get the token from redux
  const AuthToken = useSelector((state) => state.auth.currentUser.token);

  // current doctor
  const allDoctor = useSelector((state) => state.doctor.doctor);
  const currentDoctor = allDoctor.filter(
    (list) => list.doctor.id === props.doctorId
  )[0];
  ...
  const formik = useFormik({
    initialValues: {
      ...
      nric: currentDoctor.nric,
      phoneNumber: currentDoctor.phone,
      mmcRegistrationNumber: currentDoctor.doctor.mmc_registration_number,
      graduatedFrom: currentDoctor.doctor.graduated_from,
    },
    ...
    onSubmit: (values) => {
      ...
      axios
        .post(
          `${process.env.REACT_APP_BACKEND_URL}/auth/doctors/${currentDoctor.doctor.id}`,
          {
            title: values.title,
            name: values.name,
            email: values.email,
        ...
        .then(function (response) {
          ...
          props.setShowEditDoctorForm(false);
        })
  ...
  function deleteDoctor(id) {
    ...
    axios
      .delete(`${process.env.REACT_APP_BACKEND_URL}/auth/doctors/${id}`, config)
      .then(function (response) {
        console.log(response);
        window.location.reload(true);
  ...
  return (
    <div
      className="bg-white bg-opacity-90 h-screen w-screen absolute left-0 top-0 flex justify-center items-center"
      onClick={() => props.setShowEditDoctorForm(false)}
    >
      <div
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit} className="flex flex-col w-full">
          ...
          <button
            type="button"
            className="bg-white border-secondary border-2 mt-3 rounded-lg p-2 text-secondary font-bold"
            onClick={() => deleteDoctor(props.doctorId)}
          >
            Delete doctor
          </button>
        </form>
